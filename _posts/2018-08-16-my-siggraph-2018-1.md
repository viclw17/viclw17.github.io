...
# Day 4
This day was [_Epic_](https://www.unrealengine.com/en-US/events/siggraph-2018/learn-from-our-tech-experts).

<img src="{{ site.url }}/images/siggraph-7.JPG" width="480"  style="display:block; margin:auto;">
<div style="text-align:center">
Excited to get a spot in the back as the line was insane...
</div>
<br>

4 Epic sessions were filling up the whole day started with _The Technology Behind 'The Speed of Light'_. _[The Speed of Light](https://www.unrealengine.com/en-US/blog/porsche-nvidia-and-epic-games-reveal-the-speed-of-light-for-porsche-911-speedster-concept)_ is the featured project of Epic this year which showcased the latest real-time raytracing (RTRT) technology. The demo was built with Microsoft Ray Tracing API runs on two NVIDIA Quadro RTX cards. New Unreal Engine features demonstrated include:
...
unclamped [IBL](https://en.wikipedia.org/wiki/Image-based_lighting) ?

# data prep
Min Oh, TA
Car model is provided, no extra modelling needed. Models come as CAD file and are processed with [Datasmith](https://www.unrealengine.com/en-US/blog/introducing-datasmith-a-workflow-toolkit-for-unreal-engine) + DCC(Maya) + endless iteration. Batch processing was done with python scripts. Geometry and UV need to be cleaned up. Production was extremely well organized and detailed driven as the project is for showcasing an industrial design masterpiece. In the end 876 CAD parts are filtered down to 165 mesh groups which are 9.46m polygons. One of the particular important thing is to clean up all the holes on the models, because inner structrues cause unnecessary ray calculation.
