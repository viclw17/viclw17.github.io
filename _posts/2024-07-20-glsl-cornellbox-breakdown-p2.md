...
Here we can see throughput is scaled **by BRDF, by sampling pdf, and by surface cosine**.

Note that the brdf sampling `pdf` is located inside the brdf shader file, as well as the **bounced-off light direction** `wi_local`, which is very organized. 
