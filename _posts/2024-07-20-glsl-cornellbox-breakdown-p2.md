...
```
<!-- `accumTexture` and `stateTexture` texture objects are generated/setup in `renderer.h`. `accumTexture` is initialized with float number 0; `stateTexture` is initialized with unsigned random integer. -->
