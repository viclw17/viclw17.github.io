...
# Unreal Material Implementation

Fresnel Optimization

```c
//F
float3 fresnel0 = lerp(float3(0.04,0.04,0.04), albedo, 1-metallic);
float p = ((-5.55473 * ndoth) - 6.98316) * ndoth; // ndoth or ndotv???
float3 f = fresnel0 + ((1.0 - fresnel0) * pow(2.0, p));
f = saturate(f);
float3 ks  = f;
```
For Fresnel, we made the typical choice of using Schlick’s approximation [19], but with a minor modification: we use a Spherical Gaussian approximation [10] to replace the power. It is slightly more
